...
int main (int argc, char *argv[]) {
    ...
    map<wchar_t,size_t> id;
    vector<wchar_t> r_id;
    vector<map<wchar_t,size_t>> E;
    ...
    while (ifs >> str) {
        for (auto c : str) {
            if (not_punc(c) && id.find(c) == id.end()) {
                id[c] = count++;
                r_id.push_back(c);
            }
        }
        ...
        for (size_t i = 0 ; i + 1 < str.size() ; ++i) {
            if (not_punc(str[i]) && not_punc(str[i+1]) && str[i] != str[i + 1]) {
                ++E[id[str[i]]][str[i+1]];
                ++E[id[str[i+1]]][str[i]];
                if (edge.find(make_pair(id[str[i+1]],id[str[i]])) == edge.end())
                    edge.insert(make_pair(id[str[i]],id[str[i+1]]));
            }
        }
    }
    ...
    map<wchar_t,size_t> id_c;
    vector<wchar_t> r_id_c;
    vector<map<wchar_t,size_t>> E_c;
    for (size_t i = 0 ; i < E.size() ; ++i) {
        if (lc[i]) {
            id_c[r_id[i]] = r_id_c.size();
            r_id_c.push_back(r_id[i]);
            map<wchar_t,size_t> temp;
            for (auto e : E[i]) {
